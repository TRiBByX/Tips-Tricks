...
| `ps -aux` piped into `less` | List running services into less for readability. |
| `watch -n 0.5 'ps -aux'` | Watch running services at an interval of 0.5 seconds. |
| `curl ipinfo.ip/ip` | Find external ip address. |
| `cat _filename_` piped into `less` | Print file contents and pipe into Less for readability. |
